  ...
  return (
    <div className="space-y-6">
      ...
      <div className="rounded-lg border border-gray-200 bg-white p-6">
        ...
        <p className="mt-2 text-gray-700 leading-relaxed">{data.executiveSummary}</p>
        <div className="mt-4 flex gap-4">
          <span className="text-sm text-gray-500">
            Period: {new Date(data.period.start).toLocaleDateString()} — {new Date(data.period.end).toLocaleDateString()}
          </span>
          <span className="text-sm text-gray-500">{data.articleCount} articles analyzed</span>
        </div>
      </div>
      ...
      {data.emergingTopics.length > 0 && (
        <div className="rounded-lg border border-amber-200 bg-amber-50 p-5">
          <h3 className="text-sm font-semibold text-amber-800 uppercase">Emerging Topics</h3>
          <div className="mt-2 flex flex-wrap gap-2">
            {data.emergingTopics.map((topic) => (
              <span key={topic} className="rounded-full bg-amber-200 px-3 py-1 text-sm font-medium text-amber-800">
                {topic}
              </span>
            ))}
          </div>
      ...
      {data.narrativeAnalysis && (
        <div className="rounded-lg border border-gray-200 bg-white p-6">
          <h2 className="text-lg font-semibold text-gray-900">Narrative Analysis</h2>
          <div className="mt-4 space-y-4">
            {data.narrativeAnalysis.keywords && (
              <div>
                <h4 className="text-sm font-semibold text-gray-700">Keywords</h4>
                <p className="mt-1 text-sm text-gray-600 leading-relaxed whitespace-pre-line">{data.narrativeAnalysis.keywords}</p>
              </div>
            )}
            {data.narrativeAnalysis.topics && (
              <div>
                <h4 className="text-sm font-semibold text-gray-700">Topics</h4>
                <p className="mt-1 text-sm text-gray-600 leading-relaxed whitespace-pre-line">{data.narrativeAnalysis.topics}</p>
              </div>
            )}
            {data.narrativeAnalysis.technologies && (
              <div>
                <h4 className="text-sm font-semibold text-gray-700">Technologies</h4>
                <p className="mt-1 text-sm text-gray-600 leading-relaxed whitespace-pre-line">{data.narrativeAnalysis.technologies}</p>
              </div>
            )}
            {data.narrativeAnalysis.opportunities && (
              <div>
                <h4 className="text-sm font-semibold text-gray-700">Opportunities</h4>
                <p className="mt-1 text-sm text-gray-600 leading-relaxed whitespace-pre-line">{data.narrativeAnalysis.opportunities}</p>
              </div>
            )}
          </div>
        </div>
