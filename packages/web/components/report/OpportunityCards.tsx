...
import type { ContentOpportunity } from "@pulsar/shared/types";

export default function OpportunityCards({ data }: { data: ContentOpportunity[] }) {
  if (data.length === 0) {
    return <p className="text-sm text-gray-400 dark:text-neutral-500">No opportunities identified this period.</p>;
  }

  return (
    <div className="space-y-3">
      {data.map((opp, i) => (
        <div key={i} className="rounded-lg border border-gray-200 dark:border-neutral-700 border-l-4 border-l-indigo-500 bg-white dark:bg-neutral-800 p-4">
