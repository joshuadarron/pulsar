...
const OUTCOME_FILTERS = ["all", "pending", "confirmed", "partially_confirmed", "refuted", "inconclusive"];
...
export default function EvalsPage() {
	...
	return (
		<div className="space-y-8 p-8">
			...
			<section>
				<h2 className="mb-4 text-lg font-semibold text-gray-900 dark:text-neutral-100">Recent run quality (30 days)</h2>
				{runs.length === 0 ? (
					...
					<div className="overflow-x-auto rounded-lg border border-gray-200 dark:border-neutral-800 bg-white dark:bg-neutral-900">
						<table className="w-full text-sm">
							<thead className="bg-gray-50 dark:bg-neutral-800">
								<tr>
									<th className="px-4 py-3 text-left text-xs font-medium uppercase text-gray-500 dark:text-neutral-400">Date</th>
									<th className="px-4 py-3 text-left text-xs font-medium uppercase text-gray-500 dark:text-neutral-400">Status</th>
									<th className="px-4 py-3 text-right text-xs font-medium uppercase text-gray-500 dark:text-neutral-400">Report</th>
									<th className="px-4 py-3 text-right text-xs font-medium uppercase text-gray-500 dark:text-neutral-400">Drafts avg</th>
									<th className="px-4 py-3 text-right text-xs font-medium uppercase text-gray-500 dark:text-neutral-400">Validations</th>
								</tr>
							</thead>
							<tbody>
								{runs.map((r) => (
									<tr key={r.run_id} className="border-t border-gray-100 dark:border-neutral-800 hover:bg-gray-50 dark:hover:bg-neutral-800">
										<td className="px-4 py-3">
											<Link href={`/evals/${r.run_id}`} className="text-indigo-600 dark:text-indigo-400 hover:underline">
										...
										<td className="px-4 py-3 text-right tabular-nums">
											<span className="text-green-600 dark:text-green-400">{r.validations_passed}</span>
											{" / "}
											<span className={r.validations_failed > 0 ? "text-red-600 dark:text-red-400" : "text-gray-400"}>
			...
			<section>
				<h2 className="mb-4 text-lg font-semibold text-gray-900 dark:text-neutral-100">Quality trends (90 days)</h2>
				<div className="grid gap-4 lg:grid-cols-2">
					<div className="rounded-lg border border-gray-200 dark:border-neutral-800 bg-white dark:bg-neutral-900 p-4">
						<p className="mb-3 text-sm font-medium text-gray-700 dark:text-neutral-300">Trend report scores by dimension</p>
						<EvalTrendChart data={reportTrend} />
					</div>
					<div className="rounded-lg border border-gray-200 dark:border-neutral-800 bg-white dark:bg-neutral-900 p-4">
						<p className="mb-3 text-sm font-medium text-gray-700 dark:text-neutral-300">Drafts sub-check pass rate (30 days)</p>
						{draftPass.length === 0 ? (
							...
							<table className="w-full text-sm">
								<thead>
									<tr className="border-b border-gray-200 dark:border-neutral-700">
										...
										<th className="pb-2 text-right text-xs font-medium uppercase text-gray-400 dark:text-neutral-500">Pass rate</th>
										<th className="pb-2 text-right text-xs font-medium uppercase text-gray-400 dark:text-neutral-500">Pass / total</th>
									</tr>
								</thead>
								<tbody>
									{draftPass.map((d) => {
										...
										return (
											<tr key={d.platform} className="border-b border-gray-100 dark:border-neutral-800 last:border-0">
												...
												<td className="py-2 text-right tabular-nums text-gray-600 dark:text-neutral-400">{d.passed} / {d.total}</td>
											</tr>
										);
									})}
								</tbody>
					...
					<div className="rounded-lg border border-gray-200 dark:border-neutral-800 bg-white dark:bg-neutral-900 p-4 lg:col-span-2">
						<p className="mb-3 text-sm font-medium text-gray-700 dark:text-neutral-300">Validation failures by pipeline (30 days)</p>
							...
							<div className="space-y-2">
								{validationFailures.map((v) => (
									<div key={v.pipeline_name} className="flex items-center gap-3">
										<span className="w-48 text-sm text-gray-700 dark:text-neutral-300">{v.pipeline_name}</span>
										...
										<span className="w-12 text-right tabular-nums text-sm text-gray-700 dark:text-neutral-300">{v.failures}</span>
									</div>
								))}
							</div>
						)}
			...
			<section>
				<div className="mb-4 flex items-center justify-between">
					<h2 className="text-lg font-semibold text-gray-900 dark:text-neutral-100">Predictions tracking</h2>
					<select
						value={outcomeFilter}
					...
					<div className="overflow-x-auto rounded-lg border border-gray-200 dark:border-neutral-800 bg-white dark:bg-neutral-900">
						<table className="w-full text-sm">
							<thead className="bg-gray-50 dark:bg-neutral-800">
								<tr>
									<th className="px-4 py-3 text-left text-xs font-medium uppercase text-gray-500 dark:text-neutral-400">Date</th>
									<th className="px-4 py-3 text-left text-xs font-medium uppercase text-gray-500 dark:text-neutral-400">Prediction</th>
									<th className="px-4 py-3 text-left text-xs font-medium uppercase text-gray-500 dark:text-neutral-400">Type</th>
									<th className="px-4 py-3 text-left text-xs font-medium uppercase text-gray-500 dark:text-neutral-400">Status</th>
									<th className="px-4 py-3 text-left text-xs font-medium uppercase text-gray-500 dark:text-neutral-400">Evidence</th>
								</tr>
							</thead>
							<tbody>
								{predictions.map((p) => (
									<tr key={p.prediction_id} className="border-t border-gray-100 dark:border-neutral-800">
										<td className="px-4 py-3 text-gray-600 dark:text-neutral-400">
											{new Date(p.report_generated_at).toLocaleDateString()}
										</td>
										<td className="px-4 py-3 max-w-xl text-gray-900 dark:text-neutral-100">{p.prediction_text}</td>
										<td className="px-4 py-3 text-xs text-gray-500 dark:text-neutral-400">{p.prediction_type}</td>
										<td className="px-4 py-3">
											<span className={`rounded-full px-2 py-0.5 text-xs font-medium ${OUTCOME_COLORS[p.status] ?? OUTCOME_COLORS.inconclusive}`}>
												{p.status}
											</span>
										</td>
