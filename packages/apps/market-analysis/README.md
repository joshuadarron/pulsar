...
|---|---|---|
| `trend-report` | Daily at 05:30 (after the scheduled scrape completes) | Four-pass report generation. Pass 1 writes Market Snapshot and Developer Signals, leading from a week-over-week mention delta and refusing to repeat any of the previous four reports' lead sentences. Pass 2 produces Signal Interpretation as 2 to 3 prose paragraphs (`narrative: string[]`); the legacy `interpretations: SignalInterpretation[]` array shape still renders for historical reports. Pass 3 writes the executive summary, with operator-positioning tie-in capped to one sentence and omitted when the period does not intersect. Pass 4 aggregates and ranks Supporting Resources from per-section research entries. |
...
| `article-picker` / `article-writer` / `article-annotator` (article-package, three-pass) | Chained after `trend-report` | Three-pass article generation that writes to `content_articles`. The picker dedupes against the last 8 published articles, requires each spec to derive from a distinct narrative paragraph, and assigns a metaphor family plus Medium publication. The writer renders the body in markdown using operator voice samples. The annotator produces pull quotes, image prompts, and publication targets. The operator reveal section is optional per article. |
